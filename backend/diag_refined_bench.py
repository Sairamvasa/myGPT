"""Benchmark refined smart num_predict: RAG factual=160, RAG explain=320."""
...
print("=" * 90)
print("REFINED SMART NUM_PREDICT BENCHMARK")
print("=" * 90)
...
print("\n" + "-" * 90)
print(f"{'Question':<30} | {'Limit':>6} | {'TTFT':>8} | {'Total':>8} | {'Tokens':>7} | {'tok/s':>6} | {'Complete':>9} | {'Correct':>8} | {'Halluc':>10}")
print("-" * 90)
...
for name, question, uid, cid in questions:
    ...
    # Run with the smart limit
    text_smart, ttft_smart, total_smart, tok_smart, tps_smart = run(prompt, smart_limit)
    csmart, hsmart, compsmart = evaluate(text_smart, question)

    print(f"{name:<30} | {smart_limit:>6} | {ttft_smart:>7.0f}ms | {total_smart:>7.0f}ms | {tok_smart:>6} | {tps_smart:>5.1f} | {str(compsmart):>9} | {str(csmart):>8} | {','.join(hsmart) or 'none':>10}")
    print(f"  Response: {text_smart[:150]!r}")
...
print("  RAG factual/output/value questions -> 160 tokens (precise short answers)")
print("  RAG code explanation/review/analysis -> 320 tokens (complete explanations)")
print("  RAG deep/long analysis -> 512 tokens (comprehensive)")
print("  Normal short/simple questions -> 64 tokens (fast)")
print("  Normal chat -> 160 tokens")
print("  OLLAMA_NUM_PREDICT caps all results")